    ...
    <header className="border-b border-border bg-card/95 backdrop-blur supports-[backdrop-filter]:bg-card/60 sticky top-0 z-50">
      <div className="container mx-auto px-4">
        <div className="flex items-center justify-between h-16">
          ...
          <Link
            href="/"
            className="flex items-center space-x-3 hover:opacity-80 transition-all duration-300 hover:scale-105 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 rounded-md group"
            aria-label="EcoFinds Home"
          >
            <div className="relative w-10 h-10 transition-transform duration-300 group-hover:rotate-12">
              <Image src="/ecofinds-logo.png" alt="EcoFinds Logo" width={40} height={40} className="object-contain" />
            </div>
            <h1 className="text-xl font-bold text-primary transition-colors duration-300 group-hover:text-primary/80">
              EcoFinds
            </h1>
          </Link>
          ...
          <div className="flex items-center space-x-2">
            ...
            <Link href="/products/add" className="hidden sm:block">
              <Button
                size="sm"
                variant="outline"
                className="flex items-center gap-2 transition-all duration-300 hover:scale-105 hover:shadow-lg hover:bg-primary/5 bg-transparent group"
            ...
            <Sheet open={isOpen} onOpenChange={setIsOpen}>
              <SheetTrigger asChild>
                ...
                >
                  <Menu className="h-5 w-5 transition-transform duration-300 hover:rotate-90" aria-hidden="true" />
                </Button>
              </SheetTrigger>
                ...
                <nav className="flex flex-col space-y-4 mt-8" role="navigation" aria-label="Mobile navigation">
                  ...
                  <div className="border-t border-border pt-4 space-y-2">
                    <Link
                      href="/products/add"
                      onClick={() => setIsOpen(false)}
                      className="flex items-center gap-3 px-3 py-2 rounded-md hover:bg-muted transition-all duration-300"
                    >
                  ...
                  {!showAuthButtons && (
                    <div className="border-t border-border pt-4 space-y-2">
                      <Link href="/login" onClick={() => setIsOpen(false)}>
                        <Button variant="outline" className="w-full transition-all duration-300 bg-transparent">
                          Login
                        </Button>
                      </Link>
                      <Link href="/signup" onClick={() => setIsOpen(false)}>
                        <Button className="w-full transition-all duration-300">Sign Up</Button>
                      </Link>
                    </div>
                  )}
                </nav>
